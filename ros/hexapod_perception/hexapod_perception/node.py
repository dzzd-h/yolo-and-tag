...
from .segmenter import SemanticFeaturePredictor, YoloSegmenter
...
class PerceptionNode:
    def __init__(self):
        defaults = {
            "model_path": "", "front_feature_model_path": "",
            "green_feature_model_path": "", "output_frame": "base_link",
        ...
        if not p("front_feature_model_path"):
            raise RuntimeError("parameter 'front_feature_model_path' is required")
        if not p("green_feature_model_path"):
            raise RuntimeError("parameter 'green_feature_model_path' is required")
        ...
        self.front_predictor = SemanticFeaturePredictor(
            p("front_feature_model_path"), p("front_feature_confidence"),
            p("image_size"), p("device"), crop_to_region=False)
        self.green_predictor = SemanticFeaturePredictor(
            p("green_feature_model_path"), p("green_feature_confidence"),
            p("image_size"), p("device"), crop_to_region=True,
            crop_margin=float(p("green_crop_margin")))
        self.k = None
    ...
    def _frame(self, color_message, depth_message):
        ...
        front_detections = (_feature_near_mask(
            self.front_predictor(rgb, maximum=1), xiaolan_mask)
            if xiaolan_complete else [])
        ...
        green_detections = (self.green_predictor(
            rgb, xiaolan_mask, maximum=2) if xiaolan_complete else [])
