...
class YoloSegmenter:
    def __init__(self, model_path, board_class="board", xiaolan_class="platform_robot",
                 confidence=0.45, image_size=640, device="cpu", erosion_pixels=2):
        from ultralytics import YOLO

        self.model = YOLO(model_path, task="segment")
        self.board_class = board_class
        self.xiaolan_class = xiaolan_class
    ...
    def __call__(self, rgb):
        ...
        masks = {
            self.board_class: np.zeros((height, width), bool),
            self.xiaolan_class: np.zeros((height, width), bool),
        }
        scores = {self.board_class: 0.0, self.xiaolan_class: 0.0}
        ...
        masks[self.board_class] &= ~masks[self.xiaolan_class]
        return masks, scores
